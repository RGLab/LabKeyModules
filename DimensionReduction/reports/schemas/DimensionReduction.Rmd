...
# Get subjects with missing feature data for plot colored by impute
if (impute != "none") {
  missingFeaturesList <- lapply(aData, function(dt){
    missingFeatureCount <- apply(dt, 1, function(x)sum(is.na(x)))
    data.table(missing = missingFeatureCount, subject = dt$participant_id)
  })
  
  missingFeaturesDT <- Reduce(function(x,y){rbind(x,y)}, missingFeaturesList)
...
if(impute == "mean" | impute == "median"){
  ...
  aData <- lapply(aData, function(dt){ 
    
    # Transform into correct format
    extraVar <- if(aggregateBy == "subject"){dt$participant_id}else{dt$sub_tp}
    dt <- if(aggregateBy == "subject"){dt[-which(names(dt) == "participant_id")]}else{dt[-which(names(dt) == "participant_id")]}
...
subs <- lapply(aData, function(x){
  x[, grep("sub_tp|participant_id", colnames(x)), with = FALSE]
})
...
if (impute != "none") {
  ...
  demog$imputed_values <- missingFeaturesBySubject[demog$participant_id]
}
