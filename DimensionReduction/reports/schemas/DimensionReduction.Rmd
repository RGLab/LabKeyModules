...
if ( impute != "none" ) {
  ...
  missingFeatures <- lapply(aData, function(df){
    rownames(df) <- df$participant_id
    apply(df, 1, function(x)sum(is.na(x)))
  })

  missingSubjects <- lapply(aData, function(df) {
    apply(df, 2, function(x)(sum(is.na(x))))
  })
                        
                          
  imputedObservationCount <- sum(unlist(missingFeatures) != 0)
  imputedFeatureCount <- sum(unlist(missingSubjects) != 0)
  imputedValueCount <- sum(unlist(missingSubjects))
}
...
`r if ( impute != "none" ) {paste0("- **Number of Features Imputed**: ", imputedFeatureCount)}`
`r if ( impute != "none" ) {paste0("- **Number of Observations Imputed**: ", imputedObservationCount)}`
`r if ( impute != "none" ) {paste0("- **Total Number of Imputed Values**: ", imputedValueCount)}`
