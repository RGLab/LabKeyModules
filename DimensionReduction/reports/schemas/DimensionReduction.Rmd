...
getAssayData <- function(assay, con, pidsFilt, filteredPids, timePts) {
  ...
  if(assay != "gene_expression_files"){
    ...
    # Don't include SDY67 batch 2!
    if ("SDY67_Batch2_HealthyAdult" %in% mats) {
      mats <- mats[-which(mats == "SDY67_Batch2_HealthyAdult")]
    }
...
plotInfo <- list(list(plot = TRUE, name = "Age", label = "age_reported"),
                 ...
                 list(plot = (impute != "none" & max(imputed_values) > 0), name = "Imputed Values", label = "imputed_values"),
