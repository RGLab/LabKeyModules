...
add_r2 <- function(data){
  ...
  dt <- dt[, r2x := min(logFC), by = "arm_name,gene"]
  dt <- dt[, r2y := max(response), by = "arm_name,gene"]
...
#stop(paste(labkey.url.params, collapse = "\n  "))
...
if(exists("loadedCohorts") && all(loadedCohorts == arrayCohorts)){
  ...
  hai <- data.table(labkey.selectRows(baseUrl=labkey.url.base, folderPath=labkey.url.path, schemaName="study", 
                                      queryName="hai", viewName = "full", colFilter=hai_filter, colNameOpt="rname"))
...
data <- add_r2(data)
p <- ggplot(data=data, aes(x=logFC, y=response)) + geom_point(aes_string(size=size, color=color, alpha=alpha, shape=shape)) + geom_smooth(method="lm") + ylab(response) + xlab(xlab) + geom_text(aes(x=r2x, y=r2y, label = r2), hjust = 0, vjust = 1, data = data, parse = TRUE) + theme(text=element_text(size=textSize)) 
if(facet == "grid"){
