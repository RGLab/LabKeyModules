...
setcolorder(table, c("Module", "Contrast", "NGenes", "Correlation", "Direction", "PValue", "FDR"))
...
DEM <- table[Module %in% DEM, list(Module, Contrast, PValue)]
DEM <- dcast(DEM, formula = Module ~ Contrast, value.var = "PValue")
mat <- as.matrix(DEM[, -1])
...
kable(table, format='html', row.names=TRUE, table.attr='id="res_table"')
```
