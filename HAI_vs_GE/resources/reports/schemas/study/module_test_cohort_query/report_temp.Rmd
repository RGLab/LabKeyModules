...
library(knitr)
opts_chunk$set(echo=TRUE, eval=TRUE, message=FALSE, warning=TRUE, error=TRUE, fig.width=8, fig.height=4, dpi=100, fig.align="center")
...
#hai
#
GEM_filter <- makeFilter(c("expression_matrix_accession", "EQUAL", param$EM_acc))
ds_GEM <- labkey.selectRows(baseUrl = labkey.url.base, folderPath = labkey.url.path, schemaName = "study", queryName = "gene_expression_matrices", colNameOpt="rname", colFilter=GEM_filter)
GEM <- data.table(ds_GEM)
...
<<<<<<< HEAD
fit_hai_tiv08 <- glmnet(x, y, alpha=0.5)
=======
alphavalue <- as.numeric(labkey.url.params$alpha)
fit_hai_tiv08 <- glmnet(x, y, alpha=alphavalue)
>>>>>>> d8a47bc6db031412139d07d3a27bb92657809717
