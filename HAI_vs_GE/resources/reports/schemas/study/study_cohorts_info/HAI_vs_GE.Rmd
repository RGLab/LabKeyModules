...
param <- list(
    ...
    EM_pred     = "EXPM0003",
    FDR_thresh  = 0.02, # as.numeric( labkey.url.params$fdrThreshold ),
    FC_thresh   = 0     # as.numeric( labkey.url.params$fcThreshold )
)
```
...
process_hai <- function(eset){ 
  ...
  HAI <- HAI[,list(response=max(response)),by="subject_accession"]
  return(HAI)
}
OUTCOME_train <- process_hai(GEM)
```

```{r glmnet}
#glmnet
x <- t(lFC)
y <- log2(OUTCOME_train$response)
...
#Heatmap
annotation <- data.frame(hai=log2(OUTCOME_train$response))
...
if(is.null(param$EM_pred)){
  ...
  ggplot(df, aes(x=log2(observed), y=fitted)) + geom_point() + geom_smooth(method="lm") +IS_theme
} else{
  ...
  df <- data.frame(observed = c(OUTCOME_train$response, OUTCOME_pred$response),
		   ...
		   cohort = c(rep(paste("a:", param$EM_acc, "(Training)"), length(OUTCOME_train$response)),
			      rep(paste("a:", param$EM_pred, "(Training)"), length(OUTCOME_pred$response))))
  ggplot(df, aes(x=log2(observed), y=fitted)) + geom_point() + geom_smooth(method="lm") + IS_theme +
