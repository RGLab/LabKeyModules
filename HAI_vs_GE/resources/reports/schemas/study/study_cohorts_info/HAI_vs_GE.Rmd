...
opts_chunk$set(fig.align="center", fig.height=5, fig.width=8, dpi=100)
opts_chunk$set(cache=FALSE, echo=FALSE, cache.path=paste(labkey.file.root, "cache/prediction_module/", sep="/"))
options(width=80)
```
...
#GEIN <- function(EMs, pdata, DEP, timepoint){
#   common_genes <- Reduce(intersect, lapply(EMs, "[[", "gene_symbol"))
#   DEG <- lapply(EMs, function(x){
#      return(x[feature_id %in% DEP & gene_symbol %in% common_genes, gene_symbol])
#   })
...
colnames(PRED_selected) <- paste0("X", gsub(";", "_OR_", colnames(PRED_selected)))
