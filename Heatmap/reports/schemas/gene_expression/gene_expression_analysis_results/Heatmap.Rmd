...
opts_chunk$set(cache = FALSE, cache.path = file.path(labkey.file.root, "cache/Heatmap/", labkey.user.email, ""))
```
...
```{r runs}
anno_list <- mat_list <- vector("list", length(uarm))
for(runIdx in 1:length(uarm)){
