...
get_pdata <- function(EM_acc, timepoint){
  ...
  #pdata_filter <- makeFilter(c("run_dataoutputs_name", "IN", paste(EM_acc, collapse=";")))
...
```{r main, echo = FALSE, cache = FALSE}
GEAR <- get_GEAR(param$GEA_acc, param$FC_thresh, param$FDR_thresh)
pdata <- get_pdata(unique(GEAR$expression_matrix), param$timepoint)
