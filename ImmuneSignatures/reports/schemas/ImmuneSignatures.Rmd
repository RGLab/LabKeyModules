...
# labkey.url.path set by UI report pipeline = "/HIPC/IS1/"
# labkey.url.base set by UI report pipeline
onTest <- grepl("test", labkey.url.base)
con <- CreateConnection("IS1", onTest = onTest)
...
onTest <- grepl("test",labkey.url.base)
rm(labkey.url.base, labkey.url.path) # otherwise connection is still through HIPC/IS1
sdy404 <- CreateConnection("SDY404", onTest = onTest)
rawHai <- sdy404$getDataset("hai")
