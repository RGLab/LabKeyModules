...
for(sdy in studies){
  ...
  mats <- con$cache$GE_matrices$name
  ...
  rawHai <- if(sdy != "SDY80"){ con$getDataset("hai", maxRows = 10000) }else{ SDY80_rawtiterdata_v2 }
  adjHai <- adjust_hai(sdy, rawHai)

  # Age needed for figure 2 and SDY80 filtering
  dotNum <- gsub("SDY", ".", sdy, fixed = T)
  demo <- con$getDataset("demographics", maxRows = 10000)
...
pd <- rbindlist(lapply(studies, function(sdy) {
                        ...
                        demo <- con$getDataset("demographics", maxRows = 10000)
                        ge <- con$getDataset("gene_expression_files", maxRows = 10000)
...
rawHai <- con$getDataset("hai", maxRows = 10000)
