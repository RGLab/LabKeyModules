...
library(knitr)
opts_chunk$set(dev = "CairoPNG", fig.align = "left", fig.width = 15, fig.height = 15)
```
...
seracare <- data.table(labkey.selectRows(labkey.url.base, 
                                         folderPath = "HIPC/Lyoplate",
                                         "study",
                                         "cell_population_statistics"))
cytotrol <- data.table(labkey.selectRows(labkey.url.base, 
                                         folderPath  = "HIPC/Lyoplate", "lists","cytotrol statistics for power analysis"))
local_central <- data.table(labkey.selectRows(labkey.url.base, 
                                              folderPath = "HIPC/Lyoplate","lists","Local vs. Central Manual Gating - Cytotrol Data"))
...
```{r,warning=FALSE,message=FALSE,cache=FALSE,echo=FALSE,fig.height=5,include=TRUE,htmlcap='Coefficients of variability for cell populations identified in the Lyoplate panles on the Cytotrol data, comparing local vs central manual gating. The CVs are equivalen or lower for all cell populations across panels with the exception of the Memory IgD+ cell population in the B-cell panel. The IgD stain was known to be problematic for the Lyplate panels in the Cytotrol data set.'}
# Plot Cytotrol
# ggplot(CVS[Method %in% c("local", "central")]) +
#   aes(y = CV, x = Population, fill = Method) +
#   facet_wrap(~Panel, scales = "free") +
#   geom_bar(position = "dodge", stat = "identity") +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
#   ggtitle("Cytotrol Cells") +
#   scale_fill_brewer(type = "qual")
p <- ggplot(CVS[Method %in% c("local", "central")]) +
  aes(y = CV, x = Population, fill = Method) +
  facet_wrap(~Panel, scales = "free") +
  geom_bar(position = "dodge", stat = "identity") +
  scale_fill_brewer(type = "qual",
                    palette = 5,
                    labels = c("Lyophilized Cells\n(Central gating)",
                               "Lyophilized Cells\n(Local gating)")) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "bottom") + xlab("<br><br><br>Population")
# p
ggplotly(p)
```
...
p <- ggplot(CVS[Method %in% c("manual", "automated")]) + 
  aes(y = CV, x = Population, fill = Method) + 
...
p1 <- ggplot(CVS_2[plotlabel %like% "Lyophilized"]) + 
  geom_bar(aes(x = Population, y = CV, fill = plotlabel), 
           position = "dodge", 
           stat = "identity") + 
  facet_wrap(~Panel, scales = "free_x", ncol = 3) + 
  scale_fill_manual("Experiment", 
                    values = brewer.pal(n = 3, name = "Pastel2")[c(1, 2)]) + 
  theme_bw() + 
  ...
  scale_y_continuous("Coefficient of Variability (%)") + 
...
plots <- mapply(function(pnl, V, CC) {
  ...
  list(ggplot(foo) + geom_line(aes(x = SampleSize.value, y = effects, lty=S.cat,col=S.cat),lwd=1)+scale_x_continuous("Sample Size")+scale_y_continuous("Detectable Effect Size")+ggtitle(paste(strwrap(paste0("Effect Size Detected at 80% Power: ",pnl),35),collapse="\n"))+facet_wrap(~Population,scales="free")+scale_linetype("Variability",labels=c("Central: - Cross-Center","Central: Within-Center","Local Gating"))+theme_bw()+theme(legend.position="bottom")+scale_color_brewer("Variability",type="qual",labels=c("Central: - Cross-Center", "Central: Within-Center", "Local Gating")))
},pnl = as.list(levels(factor(cv.ratio$panel))), V = list(vcb, vct, vctr), CC = list(cb, ct, ctr))
```
   
```{r, echo=FALSE, warning=FALSE, message=FALSE, htmlcap='Power analysis for the Lyoplate B-cell panel.'}
# plots[[1]]
ggplotly(plots[[1]])
```

```{r, echo=FALSE, warning=FALSE, message=FALSE, htmlcap='Power analysis for the Lyoplate T-cell panel.'}
# plots[[2]]
ggplotly(plots[[2]])
```

```{r, echo=FALSE, warning=FALSE, message=FALSE, htmlcap='Power analysis for the Lyoplate T-regulatory cell panel.'}
# plots[[3]]
ggplotly(plots[[3]])
```
