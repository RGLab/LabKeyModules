...
# Get unique CreateConnection calls
#x5Filt <- "GET /project/Studies/getContainers.view?includeSubfolders=0&includeEffectivePermissions=1 HTTP/1.1"
#ISR_inits <- ISR[ (query == "ISC_study_datasets" | X5 == x5Filt) & X1 != "10.10.1.74", ]
...
```{r isr-user}
# Remove inits without email or date
#ISR_user <- ISR_inits[!is.na(X12) & !is.na(date2), list(date2, X12)]

# De-dupe to get unique user inits 
#ISR_user <- ISR_user[ !duplicated(ISR_user)]

# Group by break
tmp <- as.POSIXct(ISR_inits$date2, format="%Y-%m-%d", tz="UTC")
ISR_usersOverTime <- ISR_inits[ , Date := as.Date(cut(tmp, breaks = labkey.url.params$by)) ]
...
# prep ISR Logins
ISR_logins <- ISR_inits[ , list(date2, X12)]
setnames(ISR_logins, colnames(ISR_logins), c("Date", "Email"))
