...
from <- ifelse(labkey.url.params$from == "",
               as.Date("2016-01-01"),
               as.Date(labkey.url.params$from))
to <- ifelse(labkey.url.params$to == "",
             Sys.Date(),
             as.Date(labkey.url.params$to))
class(from) <- "Date"
class(to) <- "Date"
from <- ifelse(from > to,
               as.Date("2016-01-01"),
...
sdys <- folders %>%
  ...
  mutate(created = as.Date(created))
sdys <- bind_rows(tibble(created = as.Date("2016-01-01"),
                         count = min(sdys$count)),
                  sdys,
                  tibble(created = Sys.Date(),
