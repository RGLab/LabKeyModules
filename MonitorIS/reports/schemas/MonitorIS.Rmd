...
p <- ggplot(df, aes(x = created, y = N)) + ggtitle("New accounts over time") + 
  xlab("Date") + ylab("New accounts") + scale_x_date(labels = date_format("%m/%Y")) + theme_IS(base_size = 18)
if (labkey.url.params$plotType == "line") {
  p <- p + geom_line()
} else if (labkey.url.params$plotType == "bar") {
...
#print(readLines("/labkey/apps/apache-tomcat-8.0.36/logs/labkey.log", n = 1))
print(list.files("/labkey/apps/apache-tomcat-8.0.36"))
print(list.files("/labkey/apps"))
```
