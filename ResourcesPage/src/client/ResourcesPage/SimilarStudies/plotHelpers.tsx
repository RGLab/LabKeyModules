...
export const PlotGrid = React.memo<plotPropsList>(
  ({ plotPropsList }: plotPropsList) => {
    return (
      <div>
        <table>
          <tbody>{CreatePlotGrid(plotPropsList)}</tbody>
        </table>
      </div>
    );
  }
...
function CreateRowOfPlots(propsSet, index) {
  ...
  return <tr key={"plot-row-" + index}>{plotList}</tr>;
}

// similar studies creation func
