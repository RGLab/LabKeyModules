...
opts_chunk$set(cache.path=paste(labkey.file.root, "cache/pubmed_report_cache/", sep="/"), cache=FALSE)
options(width=80)
```
...
dat <- labkey.selectRows("test.immunespace.org", "Studies/SDY269", "immport", "study_pubmed", colNameOpt="rname")
ds_pubmed <- dat[dat$study_accession == study_acc, ]
```

<!--
#```{r get-pubmed-info, results='asis', dependson="filter", error=FALSE}
...
#  pubmed_list <- lapply(id, function(x){entrez_link(db="all", id=x, dbfrom="pubmed")})
#  ncitation <- sapply(lapply(pubmed_list, "[[", "pubmed_pubmed_citedin"), length)
...
#  res <- lapply(pubmed_list[[1]]$pubmed_pubmed[-1][1:5], get_reference, related=TRUE)
#  for(i in 1:5)
#    {
#    cat(paste0(i,". ", res[i],"\n"))
#    }
#  url_related <- paste0("http://www.ncbi.nlm.nih.gov/pubmed?linkname=pubmed_pubmed&from_uid=", ds_pubmed$pubmed_id[1])
