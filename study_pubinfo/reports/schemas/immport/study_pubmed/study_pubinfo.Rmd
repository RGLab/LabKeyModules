...
```{r get-pubmed-info, dependson="filter", cache.extra=format(Sys.time(),'%Y-%m-%H')}
# Update every month
get_reference <- function(id){
   summary <- lapply(id, function(x){ entrez_summary(db="pubmed", id=x) })
   reference <- lapply(summary, function(x){ paste(paste(x$AuthorList,collapse=", "), ". ", x$Title, " *", x$Source, "* ", x$Volume, "(", x$Issue, "), ", x$PubDate, ".", sep="")})
