...
opts_chunk$set(echo = FALSE, 
               ...
               dpi = 100, 
               autodep = FALSE)
opts_chunk$set(cache.path = paste(labkey.file.root, "cache/pubmed_report_cache/", sep="/"), 
               cache = TRUE)
options(width = 80)
```
...
```{r filter}
dat <- labkey.selectRows(baseUrl = labkey.url.base, 
                         folderPath = labkey.url.path, 
                         schemaName = "immport", 
                         queryName = "study_pubmed", 
                         colNameOpt = "rname")
dat <- dat[dat$study_accession == basename(labkey.url.path), ]
```

```{r get-pubmed-info, results='asis', dependson="filter", error=FALSE, cache.extra=format(Sys.time(),'%Y-%m')}
id <- dat$pubmed_id[grep("^[0-9]", dat$pubmed_id)]

get_reference <- function(id){
  x <- entrez_summary(db="pubmed", id=id)
  reference <- paste(paste(x$authors$name,collapse=", "), ". ", 
                     x$title, 
                     " <i>", x$source, "</i> ", 
                     x$volume, "(", x$issue, "), ", x$pubdate, ".", sep="")
  pubmed_link <- paste0("<a href='http://www.ncbi.nlm.nih.gov/pubmed/", 
                        id,"' target='_blank'>", "PMID:", id,"</a>")
  return(paste(reference, pubmed_link))
}

getCitationCount <- function(x){
  count <- length(entrez_link(db = "all", 
                              dbfrom = "pubmed", 
                              id = x)$links$pubmed_pubmed_citedin)
  return(count)
}
...
if(length(id) == 0){
  ...
  # For local dev machines the R Program Command for the R engine demands URL encoding prior to knitting
  url_citedin <- URLencode(paste0("http://www.ncbi.nlm.nih.gov/pubmed?",
                                  "linkname=pubmed_pubmed_citedin&from_uid=", id),
                           reserved = TRUE)

  cat("<h3> Data from this study have been published in the following paper(s): </h3>\n")

  citationtxt <- paste0("<li>", study_citation,
                        "</br>The article has ",
                        ifelse(ncitation > 0, 
                               paste0("<a href='", 
                                      url_citedin, 
                                      ...
                                      " citations</a>"), 
                               "0 citations"),
                        " in PubMed central.</li>", collapse="</br>")
}
```
